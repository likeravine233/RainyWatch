...
async function fetchPandaRunning(token) {
  ...
  for (const m of arr) {
    try {
      ...
      // 当前图号:按已完成图数+1(与 LP 图序同源可靠);running.position 语义存疑(0 基?曾致图2显示"第1图"),只作 games 缺失时的兜底
      const mapNum = games.length ? games.filter(g => g.status === 'finished').length + 1 : Number(running.position || 1);
...
async function fetchPandaMatchList(token) {
  ...
  const get = async (kind, sort, extra = '', perPage = 30) => {
    const res = await fetch(`https://api.pandascore.co/csgo/matches/${kind}?per_page=${perPage}&sort=${sort}${extra}`, {
  ...
  // canceled/forfeited 场次霸占整页,真实赛果被挤出分页;页大小拉到 100(免费档上限,单请求):
  // 48h 窗内一个比赛日就能打完 30+ 场,per_page=30 的滑动窗口会把更早的赛果(NAVI vs Aurora 实例)挤出去
  ...
  const [run, up, past] = await Promise.all([
    get('running', 'begin_at'), get('upcoming', 'begin_at'), get('past', '-begin_at', pastRange, 100),
  ]);
  const norm1 = (m) => {
    try {
  ...
  return {
    ...
    // 100 = past 分页上限:48h 窗内一个比赛日可打满 30+ 场,slice(0,30) 曾把更早的赛果(NAVI vs Aurora)切在管线里
    recent: all.filter(m => m.status === 'finished' && now - (m.endedAt || m.ts) < 48 * 3600e3)
      .sort((a, b) => (b.endedAt || b.ts) - (a.endedAt || a.ts)).slice(0, 100),
  };
}
...
// —— 跨源同场判定与 LP 缺口补充(recent 覆盖增强,需用户配置 PandaScore token)——
// 队名宽松等价:归一后剥 jr/junior 词根(NAVI Jr. ↔ NAVI Junior),或子串互含(Natus Vincere ↔ NAVI)
function looseName(s) {
  return norm(s).replace(/(junior|jr)$/, '');
}
// 品牌缩写 ↔ 全名的知名对(归一+互含都抓不住的,如 NAVI=Natus Vincere 的品牌更名);按需增补
const NAME_ALIASES = [['navi', 'natusvincere']];
// 学习式别名(展示名键→页链身份键):teampool 学到后同步过来,判重与队伍池显示同源。
// 不同步的后果:LG↔Luminosity、EYE↔Eyeballers 这类缩写在判重时配不上,同场行会被补成两条(重复条目实例)
let LEARNED = {};
const setLearnedAliases = (obj) => { LEARNED = obj || {}; }; // 存引用而非拷贝:teampool 的别名表原地增删(load/learn),判重侧实时同源
function eqName(x, y) {
  const a = looseName(x), b = looseName(y);
  if (!a || !b) return false;
  const la = LEARNED[a] || a, lb = LEARNED[b] || b; // 缩写键先经学习别名路由到身份键再比
  if (la === lb) return true;
  if (a === b || (a.length >= 4 && b.length >= 4 && (a.includes(b) || b.includes(a)))) return true;
  return NAME_ALIASES.some(([p, q]) => (a === p || a === q) && (b === p || b === q));
}
function pairEq(la, lb, ra, rb) {
  return (eqName(la, ra) && eqName(lb, rb)) || (eqName(la, rb) && eqName(lb, ra));
}
// 同场判定:同队对(matchKey 全等或宽松互含)且开赛时刻相差 ≤6h。时间窗只用于判重——
// 同队再战在现实赛程里 ≥6h 才会出现(GSL 决胜局同日再战属极罕见,接受并排显示);
// 信息覆盖量由 past 接口自身的 48h 窗决定,与本窗口无关
const SAME_MATCH_MAX_DT = 6 * 3600e3;
function isSameMatch(lp, panda) {
  if (lp.id && lp.id === panda.id) return true; // 同一 Panda 行的两份拷贝:列表里那份可能已被池子装饰改了名,名字比对失手会把同场行再补一条(重复条目实例);id 相等即同场
  const dt = Math.abs((lp.ts || 0) - (panda.ts || 0));
  if (dt > SAME_MATCH_MAX_DT) return false;
  return matchKey(lp.teamA?.name, lp.teamB?.name) === matchKey(panda.teamA?.name, panda.teamB?.name)
    || pairEq(lp.teamA?.name, lp.teamB?.name, panda.teamA?.name, panda.teamB?.name);
}
// recent 缺口补充:LP 主页 Results 区块容量有限,LP 未收录的已结束场次由 Panda 行补位
// (LP 行永远优先,只增不删;补位行带 panda 标记,按结束时刻归位排序)
function supplementRecent(lpRecent, pandaRecent) {
  if (!Array.isArray(lpRecent)) return lpRecent;
  const add = (pandaRecent || []).filter((r) => r.ts && !lpRecent.some((m) => isSameMatch(m, r)));
  if (!add.length) return lpRecent;
  for (const m of add) m.panda = true;
  return [...lpRecent, ...add].sort((a, b) => (b.endedAt || b.ts || 0) - (a.endedAt || a.ts || 0));
}

module.exports = { fetchPandaRunning, matchKey, findForTeams, fetchPandaMatchList, mergeFallback, isSameMatch, supplementRecent, setLearnedAliases, norm, NAME_ALIASES };

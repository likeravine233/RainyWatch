...
function parsePlayerTeam(html) {
  ...
  const label = box.find('.infobox-cell-2.infobox-description')
    .filter((_, el) => /^Team:?$/i.test($(el).text().trim())).first();
  if (!label.length) return null;
  const val = label.next();
  const a = val.find('a[href^="/counterstrike/"]').first();
  const name = cleanText(a.text());
  if (!name) return null; // 自由球员/暂无队伍
  return { name, href: abs(a.attr('href')) };
}
...
module.exports = { parseMatches, parseEventMaps, parseEvents, parseTransfers, parsePlayerTeam, parseVrs, parseEventTier, parseEventDates, parseVrsStamp };
